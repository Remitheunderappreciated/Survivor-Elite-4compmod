...
int
	g_iHunterShotDmgTeam[MAXPLAYERS + 1],                // counting shotgun blast damage for hunter, counting entire survivor team's damage
	g_iHunterShotDmg[MAXPLAYERS + 1][MAXPLAYERS + 1],    // counting shotgun blast damage for hunter / skeeter combo
	g_iHunterLastHealth[MAXPLAYERS + 1],                 // last time hunter took any damage, how much health did it have left?
	g_iHunterOverkill[MAXPLAYERS + 1],                   // how much more damage a hunter would've taken if it wasn't already dead
	g_iPounceDamage[MAXPLAYERS + 1];                     // how much damage on last 'highpounce' done

float
	g_fHunterShotStart[MAXPLAYERS + 1][MAXPLAYERS + 1],    // when the last shotgun blast on hunter started (if at any time) by an attacker
	g_fHunterTracePouncing[MAXPLAYERS + 1],                // time when the hunter was still pouncing (in traceattack) -- used to detect pouncing status
	g_fHunterLastShot[MAXPLAYERS + 1],                     // when the last shotgun damage was done (by anyone) on a hunter
	g_fPouncePosition[MAXPLAYERS + 1][3];                  // position that a hunter (jockey?) pounced from (or charger started his carry)
...
// levels / charges
float
	g_fChargeTime[MAXPLAYERS + 1],            // time the charger's charge last started, or if victim, when impact started
	g_fChargeVictimPos[MAXPLAYERS + 1][3];    // location of each survivor when it got hit by the charger
int
	g_iChargerHealth[MAXPLAYERS + 1],    // how much health the charger had the last time it was seen taking damage
	g_iChargeVictim[MAXPLAYERS + 1],     // who got charged
	g_iVictimCharger[MAXPLAYERS + 1],    // for a victim, by whom they got charge(impacted)
	g_iVictimFlags[MAXPLAYERS + 1],      // flags stored per charge victim: VICFLAGS_
	g_iVictimMapDmg[MAXPLAYERS + 1];     // for a victim, how much the cumulative map damage is so far (trigger hurt / drowning)

// pops
bool g_bBoomerHitSomebody[MAXPLAYERS + 1];    // false if boomer didn't puke/exploded on anybody
int
	g_iBoomerGotShoved[MAXPLAYERS + 1],    // count boomer was shoved at any point
	g_iBoomerVomitHits[MAXPLAYERS + 1];    // how many booms in one vomit so far
...
// smoker clears
bool
	g_bSmokerClearCheck[MAXPLAYERS + 1],    // [smoker] smoker dies and this is set, it's a self-clear if g_iSmokerVictim is the killer
	g_bSmokerShoved[MAXPLAYERS + 1];        // [smoker] set if the victim of a pull manages to shove the smoker
int
	g_iSmokerVictim[MAXPLAYERS + 1],          // [smoker] the one that's being pulled
	g_iSmokerVictimDamage[MAXPLAYERS + 1];    // [smoker] amount of damage done to a smoker by the one he pulled

// rocks
int
	g_iTankRock[MAXPLAYERS + 1],     // rock entity per tank
	g_iRocksBeingThrown[10],         // 10 tanks max simultanously throwing rocks should be ok (this stores the tank client)
	g_iRocksBeingThrownCount = 0;    // so we can do a push/pop type check for who is throwing a created rock
...
// cvars
ConVar
	g_hCvarReport,					// cvar whether to report at all
	g_hCvarReportOld,				// cvar activate discontinued reports
	g_hCvarRockName,				// cvar display name of the tank
	g_hCvarAllowMelee,				// cvar whether to count melee skeets
	g_hCvarAllowSniper,				// cvar whether to count sniper headshot skeets
	g_hCvarAllowGLSkeet,			// cvar whether to count direct hit GL skeets
	g_hCvarReportOldThresh,			// cvar damage in final shot for drawcrown-req.
	g_hCvarSelfClearThresh,			// cvar damage while self-clearing from smokers
	g_hCvarHunterDPThresh,			// cvar damage for hunter highpounce
	g_hCvarJockeyDPThresh,			// cvar distance for jockey highpounce
	g_hCvarHideFakeDamage,			// cvar damage while self-clearing from smokers
	g_hCvarDeathChargeHeight,    	// cvar how high a charger must have come in order for a DC to count
	g_hCvarInstaTime,    			// cvar clear within this time or lower for instaclear
	g_hCvarBHopMinStreak,    		// cvar this many hops in a row+ = streak
	g_hCvarBHopMinInitSpeed,    	// cvar lower than this and the first jump won't be seen as the start of a streak
	g_hCvarBHopContSpeed;   		// cvar

ConVar
	g_hCvarPounceInterrupt,    		// z_pounce_damage_interrupt
	g_hCvarChargerHealth,    		// z_charger_health
	g_hCvarWitchHealth,    			// z_witch_health
	g_hCvarMaxPounceDistance,    	// z_pounce_damage_range_max
	g_hCvarMinPounceDistance,    	// z_pounce_damage_range_min
	g_hCvarMaxPounceDamage;    		// z_hunter_max_pounce_bonus_damage;
...
public void OnPluginStart()
{
	...
	if (g_hCvarMaxPounceDistance == INVALID_HANDLE)
	{
		g_hCvarMaxPounceDistance = CreateConVar("z_pounce_damage_range_max", "1000.0", "Not available on this server, added by l4d2_skill_detect.", FCVAR_NONE, true, 0.0, false);
	}
	if (g_hCvarMinPounceDistance == INVALID_HANDLE)
	{
		g_hCvarMinPounceDistance = CreateConVar("z_pounce_damage_range_min", "300.0", "Not available on this server, added by l4d2_skill_detect.", FCVAR_NONE, true, 0.0, false);
	}
	if (g_hCvarMaxPounceDamage == INVALID_HANDLE)
	{
		g_hCvarMaxPounceDamage = CreateConVar("z_hunter_max_pounce_bonus_damage", "49", "Not available on this server, added by l4d2_skill_detect.", FCVAR_NONE, true, 0.0, false);
	}
	...
	if (IsValidInfected(victim))
	{
		...
		switch (zClass)
		{
			case ZC_HUNTER:
			{
				...
				*/
				bool isPouncing = (GetEntProp(victim, Prop_Send, "m_isAttemptingToPounce") || g_fHunterTracePouncing[victim] != 0.0 && (GetGameTime() - g_fHunterTracePouncing[victim]) < 0.001);
...
public Action Event_PlayerDeath(Handle hEvent, const char[] name, bool dontBroadcast)
{
	...
	if (IsValidInfected(victim))
	{
		...
		switch (zClass)
		{
			...
			case ZC_SMOKER:
			{
				...
				if (g_bSmokerClearCheck[victim] && g_iSmokerVictim[victim] == attacker && g_iSmokerVictimDamage[victim] >= GetConVarInt(g_hCvarSelfClearThresh))
				{
	...
	else if (IsValidSurvivor(victim))
	{
		...
		// int atkent = GetEventInt(hEvent, "attackerentid");
...
public Action Event_LungePounce(Handle event, const char[] name, bool dontBroadcast)
{
	...
	float
		fMin    = GetConVarFloat(g_hCvarMinPounceDistance),
		fMax    = GetConVarFloat(g_hCvarMaxPounceDistance),
		fMaxDmg = GetConVarFloat(g_hCvarMaxPounceDamage);
...
public Action Timer_CheckHop(Handle timer, int client)
{
...
public Action Timer_CheckHopStreak(Handle timer, int client)
{
	if (!IS_VALID_INGAME(client) || !IsPlayerAlive(client))
	{
		return Plugin_Continue;
...
public Action Timer_ChargeCarryEnd(Handle timer, int client)
{
...
public Action Timer_ChargeCheck(Handle timer, int client)
{
...
public Action Timer_DeathChargeCheck(Handle timer, int client)
{
	if (!IS_VALID_INGAME(client))
	{
		return Plugin_Continue;
...
public Action Timer_WitchKeyDelete(Handle timer, int witch)
{
	char witch_key[10];
...
public Action Timer_CheckRockSkeet(Handle timer, int rock)
{
	int  rock_array[3];
	char rock_key[10];
...
public Action Timer_BoomVomitCheck(Handle timer, int client)
{
...
stock void CheckWitchCrown(int witch, int attacker, bool bOneShot = false)
{
	...
	*/
	...
	else if (witch_dmg_array[MAXPLAYERS + WTCH_CROWNSHOT] >= GetConVarInt(g_hCvarReportOldThresh))
	{
		...
		if (GetConVarBool(g_hCvarHideFakeDamage))
		{
			...
			if (witch_dmg_array[MAXPLAYERS + WTCH_CROWNSHOT] < GetConVarInt(g_hCvarReportOldThresh))
			{
				return;
			}
		}
...
public Action Event_TonguePullStopped(Handle event, const char[] name, bool dontBroadcast)
{
	int
		...
		smoker   = GetClientOfUserId(GetEventInt(event, "smoker")),
		reason   = GetEventInt(event, "release_type");
	...
	HandleClear(attacker, smoker, victim, ZC_SMOKER, (g_fPinTime[smoker][1] > 0.0) ? (GetGameTime() - g_fPinTime[smoker][1]) : -1.0, GetGameTime() - g_fPinTime[smoker][0], (reason != CUT_SLASH && reason != CUT_KILL));
...
public Action Timer_CheckAlarm(Handle timer, int entity)
{
...
stock void PrintDebug(int debugLevel, const char[] Message, any...)
{
	char DebugBuff[256];
...
stock bool IsWitch(int entity)
{
	...
	if (!GetTrieValue(g_hTrieEntityCreated, classname, classnameOEC) || classnameOEC != OEC_WITCH)
	{
		return false;
	}
...
stock bool IS_VALID_INGAME(int client)
{
...
stock bool IS_SURVIVOR_ALIVE(int client)
{
...
stock bool IS_INFECTED_ALIVE(int client)
{
