...
/* HUD Print Colors */
const HUD_PRINTCOLOR_BEIGE = "\x01"
const HUD_PRINTCOLOR_BRIGHT_GREEN = "\x03"
const HUD_PRINTCOLOR_ORANGE = "\x04"
const HUD_PRINTCOLOR_OLIVE_GREEN = "\x05"

/*===============================*\
 *   Enumerations & Constants    *
\*===============================*/

const VSLU_VERSION = "1.0.1"
...
function __vslu_init_scmp_hooks()
{
	...
	if ( "HooksHub" in roottable )
	{
		if ("UserConsoleCommand" in roottable && roottable.UserConsoleCommand == HooksHub.UserConsoleCommand)
			delete roottable.UserConsoleCommand;
		if ("UserConsoleCommand" in g_ModeScript && g_ModeScript.UserConsoleCommand == HooksHub.UserConsoleCommand)
			delete g_ModeScript.UserConsoleCommand;

		if ("InterceptChat" in roottable && roottable.InterceptChat == HooksHub.InterceptChat)
			delete roottable.InterceptChat;
		if ("InterceptChat" in g_ModeScript && g_ModeScript.InterceptChat == HooksHub.InterceptChat)
			delete g_ModeScript.InterceptChat;

		if ("AllowBash" in roottable && roottable.AllowBash == HooksHub.AllowBash)
			delete roottable.AllowBash;
		if ("AllowBash" in g_ModeScript && g_ModeScript.AllowBash == HooksHub.AllowBash)
			delete g_ModeScript.AllowBash;

		if ("AllowTakeDamage" in roottable && roottable.AllowTakeDamage == HooksHub.AllowTakeDamage)
			delete roottable.AllowTakeDamage;
		if ("AllowTakeDamage" in g_ModeScript && g_ModeScript.AllowTakeDamage == HooksHub.AllowTakeDamage)
			delete g_ModeScript.AllowTakeDamage;

		if ("BotQuery" in roottable && roottable.BotQuery == HooksHub.BotQuery)
			delete roottable.BotQuery;
		if ("BotQuery" in g_ModeScript && g_ModeScript.BotQuery == HooksHub.BotQuery)
			delete g_ModeScript.BotQuery;

		if ("CanPickupObject" in roottable && roottable.CanPickupObject == HooksHub.CanPickupObject)
			delete roottable.CanPickupObject;
		if ("CanPickupObject" in g_ModeScript && g_ModeScript.CanPickupObject == HooksHub.CanPickupObject)
			delete g_ModeScript.CanPickupObject;
	}
